...
class RewardShaper(IRewardShaper):
    ...
    def update_vars(self, game_vars: 'np.array'):
        ...
        self.selected_weapon = game_vars[12]
    ...
    def calc_reward(self, new_game_vars: 'np.array') -> float:
        ...
        if not is_dead:
            ...
            r += 0.08 * d_health if d_health > 0 and self.health <= 20.0 else 0.0

            d_weapon = new_game_vars[12] - self.selected_weapon
            d_ammo = new_game_vars[1] - self.ammo
            if d_weapon == 0:
                # reward for ammo-pickups
                t = 0.05 * d_ammo if d_ammo > 0 else 0.0
                r += min(t, 1.0)
                # penalty for ammo decrement
                # r += 0.04 * d_ammo if d_ammo < 0 else 0.0
                r += 0.001 * d_ammo if d_ammo < 0 else 0.0
                # extra reward for picking up ammo in need
                r += 0.30 * d_ammo if d_ammo > 0 and self.ammo <= 5 else 0.0
