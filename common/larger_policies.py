...
def create_augmented_larger_cnn(n_extra_features):
    """
    ref: https://github.com/hill-a/stable-baselines/issues/133#issuecomment-561805417
    Create and return a function for augmented_larger_cnn
    ...
    def augmented_larger_cnn(scaled_images, **kwargs):
        """
        ...
        layer_1 = activ(
            conv(scaled_images, 'cnn1', n_filters=32, filter_size=8, stride=2, init_scale=np.sqrt(2), **kwargs))
        layer_2 = activ(conv(layer_1, 'cnn2', n_filters=64, filter_size=8, stride=2, init_scale=np.sqrt(2), **kwargs))
        layer_3 = tf.nn.max_pool2d(layer_2, ksize=3, strides=2, padding='VALID', name='mp1')
        layer_4 = activ(conv(layer_3, 'cnn3', n_filters=128, filter_size=4, stride=1, init_scale=np.sqrt(2), **kwargs))
        layer_5 = tf.nn.max_pool2d(layer_4, ksize=3, strides=2, padding='VALID', name='mp2')
        layer_6 = activ(conv(layer_5, 'cnn4', n_filters=192, filter_size=3, stride=1, init_scale=np.sqrt(2), **kwargs))
        layer_6 = conv_to_fc(layer_6)

        # Append direct features to the final output of extractor
        img_output = activ(linear(layer_6, 'cnn_fc1', n_hidden=1024, init_scale=np.sqrt(2)))

        concat = tf.concat((img_output, other_features), axis=1)

        return concat

    return augmented_larger_cnn


class AugLargerLnLstmPolicy(RecurrentActorCriticPolicy):
    """
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, n_lstm=512, reuse=False, layers=None,
                 net_arch=None, act_fun=tf.tanh, cnn_extractor=nature_cnn, layer_norm=True, feature_extraction="cnn",
                 n_extra_features=0,
                 **kwargs):
        # state_shape = [n_lstm * 2] dim because of the cell and hidden states of the LSTM
        super(AugLargerLnLstmPolicy, self).__init__(
        ...
        if n_extra_features > 0:
            ...
            cnn_extractor = create_augmented_larger_cnn(n_extra_features)
...
class AugLargerLnCnnLstmPolicy(AugLargerLnLstmPolicy):
    """
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch,
                 n_lstm=512, reuse=False,
                 n_extra_features=0,
                 **_kwargs):
        super(AugLargerLnCnnLstmPolicy, self).__init__(
            sess, ob_space, ac_space, n_env, n_steps, n_batch, n_lstm, reuse,
            layer_norm=True, feature_extraction="cnn",
