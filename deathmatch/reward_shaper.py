...
class RewardShaper(IRewardShaper):
    ...
    def update_vars(self, game_vars: 'np.array'):
        ...
        self.ammo = game_vars[10:16]
        self.weapon = game_vars[16:22]
        # print(self.ammo)
        # print(self.weapon)
    ...
    def calc_reward(self, new_game_vars: 'np.array') -> float:
        ...
        if not is_dead:
            ...
            d_ammo = np.sum(np.array(new_game_vars[10:16]) - np.array(self.ammo))
            # reward for ammo-pickups
            r += 0.05 * d_ammo if d_ammo > 0 else 0.0
            # penalty for ammo decrement
            r += 0.04 * d_ammo if d_ammo < 0 else 0.0

            d_weapon = np.sum(np.array(new_game_vars[16:22]) - np.array(self.weapon))
            # reward for weapon-pickups
            r += 0.20 * d_weapon if d_weapon > 0 else 0.0
...
def test_reward_shaper():
    ...
    for e in range(2):
        ...
        while not t:
            ...
            # print(s.shape)
            # print(s.dtype)
            # print(s[0, 0:len(CONSTANTS_DICT['extra_features']), -1])
