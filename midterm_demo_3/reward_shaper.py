...
class RewardShaper(IRewardShaper):
    ...
    def update_vars(self, game_vars: 'np.array'):
        ...
        self.selected_weapon = game_vars[11]
        self.selected_weapon_ammo = game_vars[12]
        # print(f'position_x: {self.position_x}')
        # print(f'position_y: {self.position_y}')
        # print(f'position_z: {self.position_z}')
        # print(f'angle: {self.angle}')
        # print(f'selected_weapon: {self.selected_weapon}')
        # print(f'selected_weapon_ammo: {self.selected_weapon_ammo}')
    ...
    def calc_reward(self, new_game_vars: 'np.array') -> float:
        ...
        if not is_dead:
            ...
            r += 10.0 if d_health > 0 else 0.0
            # penalty for health loss
            r += -20.0 if d_health < 0 else 0.0
            # extra reward for picking up health in need
            r += 50.0 if d_health > 0 and self.health <= 10.0 else 0.0

            # reward for hits
            r += 5.0 if new_game_vars[2] > self.hit_count else 0.0
            # reward for kills
            r += 100.0 if new_game_vars[3] > self.kill_count else 0.0
            # penalty for hits taken
            r += -10.0 if new_game_vars[4] > self.hits_taken else 0.0
            # reward for armor-pickups
            r += 20.0 if new_game_vars[6] > self.armor else 0.0

            switched_weapon = self.selected_weapon != new_game_vars[11]
            d_ammo = new_game_vars[12] - self.selected_weapon_ammo
            d_ammo5 = new_game_vars[1] - self.ammo5
            # reward for ammo-pickups
            r += 20.0 if d_ammo > 0 else 0.0
            # reward for ammo5-pickups (rockets)
            r += 10.0 if d_ammo5 > 0 else 0.0
            if not switched_weapon:
                # penalty for ammo decrement
                r += 5.0 * d_ammo if d_ammo < 0 else 0.0
                # extra reward for picking up ammo in need
                r += 50.0 if d_ammo > 0 and self.selected_weapon_ammo <= 3 else 0.0
            # reward for switching to rocket launcher
            switched_to_rl = self.selected_weapon != 5.0 and new_game_vars[11] == 5.0
            r += 100.0 if switched_to_rl else 0.0

            dist = self.calc_dist(new_game_vars[7], new_game_vars[8], new_game_vars[9])
            # reward for moving around
            r += 1e-2 * dist
            # penalty for staying
            r += -0.5 if dist < 8.0 else 0.0
