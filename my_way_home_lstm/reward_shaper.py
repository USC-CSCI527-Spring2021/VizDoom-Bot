...
class RewardShaper(IRewardShaper):
    ...
    def calc_reward(self, new_game_vars: 'np.array') -> float:
        ...
        if not is_dead:
            ...
            r += 0.04 if d_health > 0 else 0.0
            # penalty for health loss
            r += 0.05 * d_health if d_health < 0 else 0.0
            # extra reward for picking up health in need
            r += 0.3 if d_health > 0 and self.health <= 10.0 else 0.0

            d_ammo = new_game_vars[1] - self.ammo
            d_selected_weapon = new_game_vars[11] - self.selected_weapon
            if d_selected_weapon == 0:
                # reward for ammo-pickups
                r += 0.15 if d_ammo > 0 else 0.0
                # penalty for ammo decrement
                r += 0.02 * d_ammo if d_ammo < 0 else 0.0
                # extra reward for picking up ammo in need
                r += 0.3 if d_ammo > 0 and self.ammo <= 3 else 0.0

            # reward for hits
            r += 0.2 if new_game_vars[2] > self.hit_count else 0.0
            # reward for kills
            ...
            r += 0.05 if new_game_vars[6] > self.armor else 0.0
            ...
            # if d_angle > 300:
            #     d_angle = abs(360.0 - d_angle)
            # reward for looking around
            # r += 1e-6 * d_angle
            # penalty for consistently looking at the same direction
            # r += -0.0001 if d_angle < 10.0 else 0.0
